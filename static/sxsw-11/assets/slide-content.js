...
PhysicsBox.prototype._createWorld = function() {
  ...
  var gravity = new b2Vec2(0,this.WORLD_GRAVITY);   //new b2Vec2(0,this.WORLD_GRAVITY);
  var doSleep = true;
  delete this.WORLD;
...
PhysicsBox.prototype._getRandVelocity = function() {
  var velocity = Math.random() * 540 + 100;
...
window.addEventListener("deviceorientation", function(event) {
    window.pbEvent = event;     //console.log( event.alpha, event.beta, event.gamma)
}, false);
...
PhysicsBox.prototype._step = function() {
  ...
  var rotate, gamma = pbEvent.gamma;
  
  if( Math.abs(gamma) < 3 )
    rotate = 0;
  else
    rotate = gamma / 90 * this.WORLD_GRAVITY * 2;
  
  // change the world's gravity!!!
  pb.WORLD.m_gravity = { x: rotate , y: this.WORLD_GRAVITY };
...
DiffScheduler.prototype._scheduleDiff = function() {
  console.log('schruledd',this.RUN);
  if (this.RUN == false) {
    return;
  }
  if (this.USE_WORKER) {
...
DiffScheduler.prototype._normalGetDiff = function() {
  ...
  if ($('.workerdemo').is(':hidden')) return;
