...
Presentation.prototype.onKeyDown = function(evt) {
  ...
  if ($(document.activeElement).closest('pre,code,input,#contented').length) return true;
...
jQuery(document).ready(function() {
 ...
 // $('code,pre').attr('contenteditable','true');
});




function goBullet(next){

  var slide = $('.slide.current');
  var highlight = slide.find('li.highlight');
  if (highlight.length == 0){
    highlight = slide.find('li').addClass('dim').eq(0);
  } else {
    highlight = highlight.removeClass('highlight')[next ? 'next' : 'prev']();
  }
  highlight.addClass('highlight')
    .doOnce(workerInit)
    .find('a').attr('target','_blank').eq(0).focus();
    
  if (highlight.length == 0 ){
    slide.find('li').removeClass('dim');
  }
}
...
function workerInit(){
    ...
    return this.filter('#workeritem,#webglitem,#storageitem').doOnce(function () {
      
      var on = this[0].id == 'workeritem';
      
      // $('.workerdemo')[ on ? 'fadeIn' : 'hide' ]();
